...
<div class="flex h-[calc(100vh-8.5rem)] min-h-[550px] overflow-hidden rounded-3xl bg-[#141417] border border-[#27272a] shadow-2xl text-slate-100 transition-colors" id="chat-app-container">
    ...
    <div class="w-full md:w-80 flex flex-col border-l border-[#27272a] h-full shrink-0 md:flex p-4 bg-[#141417] overflow-hidden" id="contacts-sidebar-pane">
        ...
        <div class="relative mb-4">
            ...
            <input id="contact-search" oninput="filterContactsList()" class="w-full bg-[#1c1c20] border border-[#2d2d35] rounded-xl py-2.5 pr-10 pl-4 text-xs font-semibold text-slate-200 placeholder:text-slate-500 focus:ring-1 focus:ring-[#f2f20d] transition-all outline-none" placeholder="ابحث عن الكادر التعليمي والطلاب..." type="text"/>
        </div>
...
<style>
    ...
    /* ========================================================
       LIGHT MODE CHAT THEME OVERRIDES (الوضع النهاري لولي الأمر)
       ======================================================== */
...
<script>
    ...
    function submitMessage() {
        ...
        if (editingMessageId) {
            fetch('/parent/messages/' + editingMessageId + '/edit', {
                method: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
    ...
    function searchActiveChatMessages() {
        ...
        fetch(`/parent/messages/conversation/${activeContactId}/search?q=${encodeURIComponent(query)}`)
            .then(res => res.json())
            .then(data => {
                const feed = document.getElementById('messages-feed');
                feed.innerHTML = '';
