...
@section('content')
    ...
    <form class="space-y-5 pb-10" action="{{ route('admin.accounts.store.affairs') }}" method="POST">
        ...
        <div class="space-y-1.5">
            ...
            <div class="relative group">
                <input required name="email" class="w-full bg-white dark:bg-surface-dark border border-slate-200 dark:border-slate-700/50 rounded-2xl px-4 py-3.5 pl-10 text-slate-800 dark:text-slate-100 text-right focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all placeholder:text-slate-400 shadow-sm" dir="ltr" placeholder="affairs@edu-bridge.com" type="email" value="{{ old('email') }}"/>
