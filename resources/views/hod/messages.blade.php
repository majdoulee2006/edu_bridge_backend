...
<script>
    ...
    function loadContacts() {
        ...
        fetch("{{ route('hod.messages.contacts') }}", {
            headers: {
    ...
    function fetchMessagesFeed(contactId) {
        ...
        fetch(`/hod/messages/conversation/${contactId}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json'
            }
    ...
    function submitMessage() {
        ...
        if (editingMessageId) {
            fetch('/hod/messages/' + editingMessageId + '/edit', {
                method: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
        ...
        const fileToUpload = selectedAttachmentFile;
        ...
        xhr.open('POST', "{{ route('hod.messages.send') }}", true);
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                try {
                    ...
                    if (data.success || data.status === 'success') {
                        const newMsg = data.message || data.data;
                        if (newMsg && !document.getElementById('msg-' + newMsg.id)) {
                            appendMessageBubble(newMsg);
                            scrollMessagesToBottom();
                        }
                        loadContactsSilently();
                    } else {
                        alert('فشل في إرسال الرسالة: ' + (data.error || data.message || 'خطأ غير معروف'));
                    }
                } catch (e) {
                    alert('حدث خطأ أثناء معالجة استجابة الخادم.');
                }
            } else {
                alert('فشل إرسال الرسالة. خطأ الخادم: ' + xhr.status);
            }
        };
    ...
    function searchActiveChatMessages() {
        ...
        fetch(`/hod/messages/conversation/${activeContactId}/search?q=${encodeURIComponent(query)}`)
            .then(res => res.json())
            .then(data => {
                const feed = document.getElementById('messages-feed');
                feed.innerHTML = '';
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                try {
                    ...
                    if (data.success || data.status === 'success') {
                        ...
                        alert('فشل إرسال الملاحظة الصوتية: ' + (data.error || data.message || 'خطأ غير معروف'));
                    }
                } catch (e) {
                    alert('حدث خطأ أثناء معالجة استجابة الخادم.');
                }
