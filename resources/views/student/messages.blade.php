...
<script>
    ...
    function loadContacts() {
        ...
        fetch("{{ route('student.messages.contacts') }}", {
            headers: {
    ...
    function fetchMessagesFeed(contactId) {
        ...
        fetch(`/student/messages/conversation/${contactId}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json'
            }
            ...
            if (fileType.startsWith('image/')) {
                ...
                mediaHtml = `
                    <div class="relative mt-2 max-w-[260px]">
                        ${pendingOverlay}
                        <a href="${url}" target="_blank" class="flex items-center gap-2 p-2 rounded-xl bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 hover:bg-black/10 text-xs font-bold transition-all truncate">
    ...
    function submitMessage() {
        ...
        if (editingMessageId) {
            fetch('/student/messages/' + editingMessageId + '/edit', {
                method: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
        ...
        xhr.open('POST', "{{ route('student.messages.send') }}", true);
    ...
    function searchActiveChatMessages() {
        ...
        fetch(`/student/messages/conversation/${activeContactId}/search?q=${encodeURIComponent(query)}`)
            .then(res => res.json())
            .then(data => {
                const feed = document.getElementById('messages-feed');
                feed.innerHTML = '';
