    ...
    <div class="flex-1 flex flex-col h-full bg-slate-50/50 dark:bg-slate-950/20 relative hidden md:flex" id="chat-room-pane">
        ...
        <div id="active-chat-window" class="flex flex-col h-full hidden">
            ...
            <div class="p-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 transition-colors flex flex-col gap-2 relative">
                ...
                <div id="attachment-preview-container" class="hidden bg-slate-100 dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700/80 rounded-2xl p-2.5 shadow-sm flex items-center justify-between gap-3 w-full mb-1 transition-all">
                    <div class="flex items-center gap-2.5 min-w-0">
                        <div class="w-9 h-9 rounded-xl bg-[#FFCC00]/20 flex items-center justify-center text-yellow-700 dark:text-yellow-400 shrink-0" id="preview-icon">
                    ...
                    <button type="button" onclick="clearSelectedAttachment()" class="w-7 h-7 rounded-full bg-slate-200/60 dark:bg-slate-700/60 flex items-center justify-center text-slate-500 hover:text-red-500 transition-colors shrink-0" title="إلغاء الملف">
                        <span class="material-symbols-outlined text-sm">close</span>
                    </button>
                </div>
...
<script>
    ...
    function loadContacts() {
        ...
        fetch("{{ route('affairs.messages.contacts') }}", {
            headers: {
    ...
    function fetchMessagesFeed(contactId) {
        ...
        fetch(`/affairs/messages/conversation/${contactId}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json'
            }
    ...
    function appendMessageBubble(msg) {
        ...
        const bubbleHtml = `
            <div ${bubbleId} class="group flex ${alignClass} w-full ${pendingClass} mb-3.5">
                ${isMe ? optionsHtml : ''}
    ...
    function submitMessage() {
        ...
        if (editingMessageId) {
            fetch('/affairs/messages/' + editingMessageId + '/edit', {
                method: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
        ...
        xhr.open('POST', "{{ route('affairs.messages.send') }}", true);
    ...
    function searchActiveChatMessages() {
        ...
        fetch(`/affairs/messages/conversation/${activeContactId}/search?q=${encodeURIComponent(query)}`)
            .then(res => res.json())
            .then(data => {
                const feed = document.getElementById('messages-feed');
                feed.innerHTML = '';
